...
const HistoryMap = (() => {
    ...
    dossierEl.innerHTML = `
      <div class="dossier-card reveal in">
        ...
        <div class="dossier-actions">
          <a href="period.html?id=${s.periodId}" class="btn btn-primary btn-sm">Explore Related Period (${s.periodId}) &rarr;</a>
          <a href="graph.html?q=${encodeURIComponent(s.name)}" class="btn btn-outline btn-sm">Inspect in Knowledge Graph &rarr;</a>
        </div>
      </div>`;
  }
  ...
  function init() {
    ...
    if (siteParam) {
      ...
      // Select Dholavira by default for immediate preview
      selectSite('dholavira');
    }
  }

  return { init, getSites, setLayer, setSearch, selectSite };
})();
