...
**69 periods** in total: **49 in the national chronology** (Palaeolithic Age through India 2020–Present, plus five World History pages) and **20 in the dedicated Uttar Pradesh / UPPSC track** (see above).
...
Uttar Pradesh track: UP History Overview → Prehistoric & Early Settlements → Vedic Age → Mahajanapadas → Maurya Period → Post-Maurya Period → Gupta Period → Harsha Period → Gurjara-Pratiharas → Kalachuris & Chandellas → Gahadavalas → Foundation of Cities in the Medieval Period → Delhi Sultanate Period → Sayyid & Lodi Dynasties → Jaunpur Sultanate → Mughal Period → Later Mughals → Revolt of 1857 → British Rule & Administration → Socio-Religious Movements.

This matches every supplied infographic image across all upload batches; alternate/expanded versions of the same period were merged into one richer entry (e.g. the two Babur, Akbar and Jahangir sheets; the two Independent-India "Major Phases" overview sheets; the Indian Freedom Struggle page, which draws on both `freedom_struggle.png` and the closely overlapping `movements_national.png`). The five World History pages carry their own `world` category and are flagged as global background rather than part of India's national chronology; the twenty Uttar Pradesh pages carry `region: "up"` and their own `up` category, and are flagged throughout as a UPPSC-focused regional companion rather than a duplicate of the national story.
